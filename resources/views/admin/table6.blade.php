...
<div class="content-wrapper">
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Table 6 Orders</h1>
            ...
            <div class="card">
                ...
                <div class="card-body table-responsive p-0">
                    <table class="table table-hover text-nowrap">
                      ...
                      <tbody>
                        @if ($table6Orders)
                          @foreach ($table6Orders as $table6Order)
                          <tr>
                              <td>{{$table6Order->name}}</td>
                              <td>{{$table6Order->category}}</td> 
                              <td>{{$table6Order->amount}}</td>
                              <td>{{$table6Order->quantity}}</td>
                              <td>{{$table6Order->created_at->format('F j, Y g:ia')}}</td>
                              <td>{{$table6Order->status}}</td>
                          </tr>
                      </tbody>
                      @endforeach  
                      @endif
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('admin.addOrder6') }}" method="post">
                  @csrf
                    <div class="form-group">
                        <label>Table Name</label>
                        <input type="text" class="form-control" name="table" value="6" readonly>
                    </div>
